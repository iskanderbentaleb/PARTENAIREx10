...
export default function SalesCreatePage({ investors, purchaseItems }: Props) {
  ...
  // Memoized available purchase items
  ...
  const debouncedSearch = useCallback(
    debounce((term: string) => {
      ...
      const results = availablePurchaseItems.filter(item =>
        item.barcode_generated.toLowerCase().includes(term.toLowerCase()) ||
        item.product_name.toLowerCase().includes(term.toLowerCase()) ||
        item.barcode_prinsipal.toLowerCase().includes(term.toLowerCase())
      ).slice(0, 5);
  ...
  useEffect(() => {
    ...
    const discount = data.discount_value || 0;
    const newTotal = Math.max(0, newSubtotal - discount);

    setData({
      ...data,
    //   subtotal: parseFloat(newSubtotal.toFixed(2)),
      subtotal: parseFloat(Number(newSubtotal).toFixed(2)),
      total: parseFloat(newTotal.toFixed(2)),
    });
  }, [data.items, data.discount_value]);
  ...
  const addItemFromSearch = (purchaseItem: PurchaseItem) => {
    if (data.items.some(item => item.purchase_item_id === purchaseItem.id)) {
    ...
    const newItem: SaleItemForm = {
      ...
      subtotal: purchaseItem.sale_price,
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <Card>
            <CardHeader className="pb-4">
              ...
              <CardDescription>
                Search for products by barcode or name. {availablePurchaseItems.length} products available in stock.
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-4">
              ...
              <CardDescription>
                {data.items.length} item(s) added to this sale
              </CardDescription>
            </CardHeader>
            <CardContent>
              {data.items.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground border-2 border-dashed rounded-lg">
                  ...
                  <p className="text-sm">Search for products above to add them to the sale</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {data.items.map((item, index) => {
                    const availableQty = getAvailableQuantity(item.purchase_item_id);
                    const maxQuantity = Math.min(availableQty, 9999); // Reasonable upper limit

                    return (
                      <div key={index} className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start p-4 border rounded-lg bg-gray-50/50">
                        {/* Product Info */}
                        ...
                        {/* Actions */}
                        <div className="lg:col-span-2 flex items-end justify-end lg:justify-start">
                          <Button
                            type="button"
                            variant="destructive"
                            size="icon"
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-semibold">
                    {formatCurrency(data.subtotal)} DZD
                  </span>
                </div>
